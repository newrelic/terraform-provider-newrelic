# GCP Dimensional Metrics — project-level IAM

Links one or more GCP projects to New Relic for the **GCP Dimensional Metrics** (keyless / Workload Identity Federation) integration, granting the required IAM roles **directly on each project** — no folder-level access required.

**Use this module when** you cannot (or prefer not to) bind IAM at the folder level. If you have folder-level access and want a single binding to cover every project under a folder, use [`gcp-dimensional-metrics-folder-iam`](../gcp-dimensional-metrics-folder-iam) instead.
...
- **Project-level IAM bindings** on each project granting the roles the integration needs:
  `roles/viewer`, `roles/serviceusage.serviceUsageConsumer`, `roles/cloudasset.viewer`, and `roles/browser` (project-level resource discovery in place of the folder-level `roles/resourcemanager.folderViewer`).
...
- GCP credentials with permission to create the WIF pool/SA in `gcp_sa_project_id` and to bind IAM on each project in `gcp_projects`.

## Usage

```hcl
module "newrelic_gcp_dm" {
  source = "github.com/newrelic/terraform-provider-newrelic//examples/modules/cloud-integrations/gcp-dimensional-metrics-project-iam"
...
| `gcp_projects` | map(string) | — | `display-name => project ID` for each project to link. IAM is bound directly on each project. The display-name becomes the linked-account name in New Relic. |
