...
<b style="color:darkblue;">During the EOL window (before July 27, 2026):</b> The Synthetics API will continue to accept legacy runtime versions. However, we strongly recommend upgrading to the new runtime as soon as possible to ensure compatibility and avoid disruptions.

<b style="color:darkred;">After the EOL date (July 27, 2026):</b> All monitors running on the legacy runtime will be automatically force-upgraded to the latest runtime version by default. The Synthetics API will enforce the use of `LATEST` for Chrome-based monitors and the `22.20.0` Node.js version for API-based monitors, and will reject any requests attempting to use legacy runtime values.
...
Since the end-of-life will take place on July 27, 2026; the following guide outlines the actions to be taken by customers who continue to maintain monitors running on the legacy runtime via Terraform, to ensure a smooth transition and avoid such drift, errors and similar unforeseen repercussions.
...
- **EOL Timeline for Scripted API Monitors:**
  - **During the EOL window (before July 27, 2026):** The Synthetics API will continue to accept legacy runtime versions (Node.js 16).
  - **After the EOL date (July 27, 2026):** All Scripted API monitors will be force-upgraded and the API will enforce `NODE_API 22.20.0` by default, regardless of their current configuration.
- **EOL Timeline for Scripted Browser Monitors:**
  - After the EOL notice period ends on July 27, 2026, all Scripted Browser monitors will be force-upgraded to `CHROME_BROWSER LATEST` regardless of their current configuration.
