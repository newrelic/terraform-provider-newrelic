...
To continue managing these rules via Terraform, the appropriate target resource depends on the `action` type of the original [`newrelic_nrql_drop_rule`](/providers/newrelic/newrelic/latest/docs/resources/nrql_drop_rule) configuration:

- For `action` types `drop_data` and `drop_attributes`, migrate to the [`newrelic_pipeline_cloud_rule`](/providers/newrelic/newrelic/latest/docs/resources/pipeline_cloud_rule) resource.
- For `action` type `drop_attributes_from_metric_aggregates`, migrate to the [`newrelic_metric_pruning_rule`](/providers/newrelic/newrelic/latest/docs/resources/metric_pruning_rule) resource, which provides equivalent functionality for reducing metric cardinality at the attribute level.
