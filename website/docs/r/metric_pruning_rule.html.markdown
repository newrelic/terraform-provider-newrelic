...
-> **Migrating from `newrelic_nrql_drop_rule`?** If you are migrating [`newrelic_nrql_drop_rule`](/providers/newrelic/newrelic/latest/docs/resources/nrql_drop_rule) resources with `action = "drop_attributes_from_metric_aggregates"`, `newrelic_metric_pruning_rule` is the recommended replacement. Please refer to the [Drop Rules EOL Migration Guide](/providers/newrelic/newrelic/latest/docs/guides/drop_rules_eol_guide) for more details.
