...
!> **NOT SUPPORTED IN THE JP REGION.** The `newrelic_alert_channel` resource is backed by the legacy REST v2 Alert Channels API (`alerts_channels.json`), which has been deprecated globally since 2024 and is **not being provisioned on the Japan (JP) region**. Configurations that set `provider "newrelic" { region = "JP" }` will fail at **`terraform plan`** time with an actionable error. Migrate JP configurations to [`newrelic_notification_destination`](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs/resources/notification_destination) + [`newrelic_notification_channel`](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs/resources/notification_channel) — both are backed by NerdGraph and are available in every region.
