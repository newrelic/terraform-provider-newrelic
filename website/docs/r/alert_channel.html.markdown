...
-> **Note - Not available in the JP region.** This resource is backed by the legacy REST v2 Alert Channels API, which is not provisioned on the New Relic Japan region. If your provider is configured with `region = "JP"`, a `terraform plan` will fail with an actionable error message. For JP configurations, use [`newrelic_notification_destination`](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs/resources/notification_destination) and [`newrelic_notification_channel`](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs/resources/notification_channel) instead - both are NerdGraph-based and available in every region.
