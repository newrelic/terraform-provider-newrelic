...
* `runtime_type` - (Optional) The runtime that the monitor will use to run jobs. For `SCRIPT_API` monitor type, valid value is `NODE_API`. For `SCRIPT_BROWSER` monitor type, valid value is `CHROME_BROWSER`.
