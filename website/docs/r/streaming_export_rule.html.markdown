...
Streaming export rules can be imported using the rule ID.

```
terraform import newrelic_streaming_export_rule.foo <rule_id>
```

~> **NOTE:** The `account_id` must be set in the resource configuration or via the provider default when importing.
