	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Test: Import
			{
				...
				ImportStateVerifyIgnore: []string{"entity_version"}, // Ignore internal, computed attribute
			},
		},
	})
}
...
func testAccCheckNewRelicPipelineCloudRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.EntityManagement.GetEntityWithContext(context.Background(), rs.Primary.ID)
...
func testAccCheckNewRelicPipelineCloudRuleExists(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		client := testAccProvider.Meta().(*ProviderConfig).NewClient
		resp, err := client.EntityManagement.GetEntityWithContext(context.Background(), rs.Primary.ID)
		if err != nil {
		...
		if _, ok := (*resp).(*entitymanagement.EntityManagementPipelineCloudRuleEntity); !ok {
